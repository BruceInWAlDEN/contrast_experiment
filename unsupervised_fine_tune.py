'''
对无监督模型进行细调 使用部分标签 0-1 的标签 模型保存为unsupervised_fine_tuned_by_0-1_epoch_{}.pth
'''
'''
本脚本用于监督训练， 直接获得模型
'''
...
parser.add_argument('--weight', default="check/unsupervised_epoch_2.pth", type=str)
...
def main():
    ...
    if args.weight:
        ...
        pretrained_dict = pth
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and k not in ['module.backbone.fc.2.bias', 'module.backbone.fc.2.weight']}
        model_dict.update(pretrained_dict)
        model.load_state_dict(model_dict)
        print('weight {} loaded !'.format(args.weight))

    # data
    train_data = MyDataset('data', s=0, e=1)  #  0-4 4-8 8-12 12-16       16-20  20-22
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(pth, 'check/unsupervised_fine_tuned_by_0-1_epoch_{}.pth'.format(epoch+1))
        ...
        # save model
        torch.save(metric, 'check/unsupervised_fine_tuned_by_0-1_epoch_{}_metric'.format(epoch+1))
